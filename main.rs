...
pub fn main() {
    ...
    main_window.on_open_image_clicked({
        let main_window_weak = main_window.as_weak();
        move || {
            let main_window = main_window_weak.unwrap();
            if let Some(file_path) = FileDialog::new().add_filter("Images", &["jpg", "png"]).pick_file() {
                let _file_url = format!("file://{}", file_path.display());
        
              
                if let Ok(image) = slint::Image::load_from_path(&file_path) {
                    main_window.set_selected_image(image);
                    {
                        let _this: &MainWindow = &main_window;
